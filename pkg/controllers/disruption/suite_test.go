...
var _ = Describe("Simulate Scheduling", func() {
	...
	It("should allow pods on deleting nodes to reschedule to uninitialized nodes", func() {
		...
		candidate, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, stateNode, pdbs, nodePoolMap, nodePoolToInstanceTypesMap, queue, disruption.GracefulDisruptionClass, cloudProvider.RepairPolicies())
		Expect(err).To(Succeed())
...
var _ = Describe("Candidate Filtering", func() {
	...
	It("should not consider candidates that have do-not-disrupt pods scheduled and no terminationGracePeriod", func() {
		...
		_, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.DeepCopyNodes()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.GracefulDisruptionClass, cloudProvider.RepairPolicies())
	...
	It("should consider candidates that have do-not-disrupt pods scheduled with a terminationGracePeriod set for eventual disruption", func() {
		...
		Expect(cluster.DeepCopyNodes()).To(HaveLen(1))
		c, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.DeepCopyNodes()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.EventualDisruptionClass, cloudProvider.RepairPolicies())
		Expect(err).ToNot(HaveOccurred())
	...
	It("should not consider candidates that have do-not-disrupt pods scheduled without a terminationGracePeriod set for eventual disruption", func() {
		...
		Expect(cluster.DeepCopyNodes()).To(HaveLen(1))
		_, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.DeepCopyNodes()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.EventualDisruptionClass, cloudProvider.RepairPolicies())
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal(fmt.Sprintf(`pod has "karpenter.sh/do-not-disrupt" annotation (Pod=%s)`, client.ObjectKeyFromObject(pod))))
		Expect(recorder.DetectedEvent(fmt.Sprintf(`Pod has "karpenter.sh/do-not-disrupt" annotation (Pod=%s)`, client.ObjectKeyFromObject(pod)))).To(BeTrue())
	})
	...
	It("should consider candidates that have do-not-disrupt terminating pods", func() {
		...
		Expect(cluster.DeepCopyNodes()).To(HaveLen(1))
		c, err := disruption.NewCandidate(ctx, env.Client, recorder, env.Clock, cluster.DeepCopyNodes()[0], pdbLimits, nodePoolMap, nodePoolInstanceTypeMap, queue, disruption.GracefulDisruptionClass, cloudProvider.RepairPolicies())
		Expect(err).ToNot(HaveOccurred())
		Expect(c.NodeClaim).ToNot(BeNil())
		Expect(c.Node).ToNot(BeNil())
	})
