...
func (d *Drift) ComputeCommands(ctx context.Context, disruptionBudgetMapping map[string]int, candidates ...*Candidate) ([]Command, error) {
	...
	for _, candidate := range slices.Concat(emptyCandidates, nonEmptyCandidates) {
		...
		if terminateFirst {
			// Delete-only (no Replacements): carry the pass-1 Results so existing nodes that can absorb the freed pods
			// get nominated; pods that need the freed reservation pend and reactive provisioning handles them.
			return []Command{{
