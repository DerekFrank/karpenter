...
func (o *Options) AddFlags(fs *FlagSet) {
	...
	fs.StringVar(&o.FeatureGates.inputStr, "feature-gates", env.WithDefaultString("FEATURE_GATES", "NodeRepair=false,ReservedCapacity=true,SpotToSpotConsolidation=false,NodeOverlay=false,StaticCapacity=false,CapacityBuffer=false,TerminateFirst=true"), "Optional features can be enabled / disabled using feature gates. Current options are: NodeRepair, ReservedCapacity, SpotToSpotConsolidation, NodeOverlay, StaticCapacity, CapacityBuffer, and TerminateFirst.")
}
...
func DefaultFeatureGates() FeatureGates {
	return FeatureGates{
		...
		TerminateFirst:          true, // terminate-first only activates where replace-first cannot run, so on by default
	}
}
